...
4. 程序总是从main 函数开始执行的 T
5. 假设赋值运算符的优先级比算术运算符高，执行以下程序段后，n的值为10。 T
    ```c
    int n; 
    n = 10 + 2;
    ```
6. 如果变量已经正确定义，则执行以下程序段后，x的值不变。F
    ```c
    if (x = 20) {
        y = 1;
    } else {
    ...
    ```
7. 输入scanf匹配：&lf->double;"x=%lf"->也要输入'x='
8. 输入scanf记得 **'&'**
9.  要记得初始化
    执行以下程序段，sum的值是55 F
    ```c
...
2.  math.h库
    ...
    log(x) -> lnx
    log10(x)
3.  编译预处理命令#include <stdio.h>的的作用并不是将标准输入输出库函数的实现代码(定义)包含到当前源文件里，而是**引入标准输入输出库的声明**。
...
9. 函数一次~~可以~~返回两个以上的值。
10. 函数未被调用时，系统将不为形参分配内存单元
11. 实参和形参的个数必须相等，但类型不一定一致，float类型的实参可以传递给double类型的形参。
12. 在C语言程序中，若对函数类型未加显式说明，则函数的隐含类型为**int**
13. 实参与其对应的形参各占用独立的存储单元
...
15. 储存，初始化
    ...
    - static char s[6] = "Happy";
16. 字符数组中的字符串可以整体输入、输出： scanf("%s",str); 不能读取空格或回车，将字符串和'\0'纳入数组
17. gets(str); 读入回车结束，将字符串和'\0'纳入数组。使用时其他scanf要把'\n'都读入，不然会影响。
    gets(p); puts(p); 里面是数组名或者地址

18. 输出单个字符，putchar(s[8]);
    ```c
    for (int i = 0; s[i] != '\0'; i++){
        purchar(s[i]);
    } //未知字符串长度，输出
    ```
19. char str[81]; 该数组可以存放81个字符型数据
20. 数组定义后，只能引用单个的数组元素，而不能一次引用整个数组
21. 可以在赋值语句中通过赋值运算符"="对字符数组整体赋值 **错！！！**
22. index不可以有负数
23. `str[len++] = ch;` 后len+1
    `str[++len] = ch;` 先len+1
24. 字符串复制。以下程序段的功能是：将字符串str1的内容复制到字符串str2。
    int i;
    ...
    `str2[i] = '\0';`
25. 删除字符串中的空格（使用1个数组）。以下程序段的功能是：将字符串str中的所有空格都删除。
    int i, j
    char str[81];
